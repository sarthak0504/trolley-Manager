...
export default function ClientDetailsScreen() {
  ...
  const getPaymentsColRef = () =>
    collection(db, `users/${client.userId}/clients/${id}/payments`);
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(clientRef, (snap) => {
      if (!snap.exists()) return;

      const updatedClient = { id: snap.id, ...snap.data() };
  ...
  const handleSavePayment = async (details) => {
    ...
    try {
      if (details.id) {
        ...
        if (oldPayment) {
          const diff = details.amount - oldPayment.amount;

          await updatePayment(id, details.id, details, oldPayment.amount);

          await updateDoc(clientRef, {
            pendingAmount: (client.pendingAmount || 0) - diff,
          });
        }
      } else {
        // ADD PAYMENT
        ...
        await updateDoc(clientRef, {
          pendingAmount: (client.pendingAmount || 0) - details.amount,
        });
      }
              ...
              [
                { text: "No", style: "cancel" },
                {
                  ...
                  onPress: async () => {
                    ...
                    await deletePayment(id, item.id, item.amount);

                    await updateDoc(clientRef, {
                      pendingAmount: (client.pendingAmount || 0) + item.amount,
                    });
                  },
                },
              ],
  ...
  const filteredAndSortedPayments = useMemo(() => {
    ...
    if (filterStartDate || filterEndDate) {
      const start = filterStartDate
        ? parseDateForPicker(filterStartDate)
        : null;
      ...
      list = list.filter((payment) => {
        ...
        if (adjustedEnd && paymentDate.getTime() > adjustedEnd.getTime())
          return false;
    ...
    return list.sort((a, b) => {
      const dA = dateStringToDate(a.date);
      const dB = dateStringToDate(b.date);
  ...
  const downloadPdf = async () => {
    ...
    try {
      ...
      if (Platform.OS === "web") {
        Alert.alert(
          "Unsupported",
          "PDF download is only supported on native (iOS/Android).",
        );
        return;
      }
  ...
  const handleMarkReturned = (rental) => {
    ...
    Alert.alert(
      "Confirm Return",
      `Proceed to return trolley ${rental.trolleyNo}?`,
      [
        { text: "Cancel", style: "cancel" },
        {
          text: "Continue",
          onPress: () => {
            router.push({
              pathname: "/client/returnPayment",
              params: {
                trolleyId: rental.trolleyNo,
                clientId: client.id,
                userId: client.userId,
                clientName: client.name,
                monthlyRent: rental.monthlyRent || 0,
                pending: rental.pending || 0,
                startDate: rental.startDate || "",
                nextRentDueDate: rental.nextRentDueDate || "",
              },
            });
          },
        },
  ...
  // --- TOTAL PAID: Only count REAL money received ---
  const totalPaidAmount = useMemo(() => {
    if (!payments.length) return 0;

    return payments.reduce((sum, p) => {
      // Count only REAL payments from customer
      if (p.type === "manual" || p.type === "advance_manual") {
      ...
      >
        ...
        {(client.jamamnatDaar || client.jamanatPhone || client.advance > 0) && (
          ...
          >
            ...
            {client.jamanatPhone && (
              <TouchableOpacity onPress={() => handleCall(client.jamanatPhone)}>
                <Text style={{ color: "#007AFF" }}>
                  📱 Phone: {client.jamanatPhone}
                </Text>
              </TouchableOpacity>
            )}
            {client.initialAdvance > 0 && (
              <Text>💰 Advance Paid: ₹{client.initialAdvance}</Text>
            )}
          </View>
        )}
        ...
        {Array.isArray(client.activeRentals) &&
        client.activeRentals.filter((r) => r && r.trolleyNo).length > 0 ? (
          client.activeRentals
            .filter((r) => r && r.trolleyNo) // remove empty rentals
            .map((r, index) => (
              <View
                key={`${r.trolleyNo}-${index}`} // ⭐ ALWAYS UNIQUE
                style={{
        ...
        >
          ...
          <Text style={{ fontSize: 16 }}>
            Advance Paid (Initial): ₹{client.initialAdvance || 0}
          </Text>

          <Text style={{ fontSize: 16 }}>
            Advance Remaining: ₹{client.advance || 0}
          </Text>
          ...
          filteredAndSortedPayments.map((item) => (
            ...
            >
              <View style={styles.paymentContent}>
                ...
                <Text style={styles.paymentDateText}>Date: {item.date}</Text>
                <Ionicons name="ellipsis-vertical" size={18} color="#6c757d" />
              </View>
              {item.notes && (
                <Text style={styles.paymentNotesText}>Notes: {item.notes}</Text>
              )}
            </TouchableOpacity>
          ))
        ) : (
